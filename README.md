...
The mod is primarily built for Windows, with an in-progress Linux compatibility layer. On Linux it reads the native PCSX2 executable's ELF exports to find EE RAM (`EEmem`) instead of using the Windows `pcsx2_offsetreader.dll`. PAL support is handled automatically at runtime.
